...
起動後にやることは登録だけ（HTTPはトークン認証。値は `tamo token` で表示され、serve起動バナーにもコピペ可能な形で出ます）:

```bash
claude mcp add --transport http tamo http://127.0.0.1:8788/mcp --header "X-Tamo-Token: $(tamo token)"   # Claude Code
# stdio派（クライアントが子プロセス起動、トークン不要）: claude mcp add tamo -- tamo mcp
```
...
```bash
tamo serve   # 常駐(推奨): claude mcp add --transport http tamo http://127.0.0.1:8788/mcp --header "X-Tamo-Token: $(tamo token)"
# stdio派:   claude mcp add tamo -- tamo mcp   (Cursor/Codex CLI等も同じ。トークン不要)
```
...
`tamo hook` が出すCLAUDE.mdスニペットと `/recall` コマンド雛形を入れると、
エージェントが「あの件どうなってた？」で自発的にtamoを引くようになります。
...
| **アクセス制御** | inbox/MCP(HTTP)ともに `127.0.0.1` バインド + **X-Tamo-Tokenトークン認証**（タイミングセーフ比較）。localhostバインドは「LANに出さない」であって認証ではないため、読出(MCP)も書込(inbox)と同じトークンで守る。`~/.tamo` は 0700、トークンは 0600（POSIX。Windowsはユーザープロファイル配下+OSのアカウント分離に依存） |
...
tamo/
  ...
  store.py       SQLite(WAL)・FTS5 trigram・quarantine・prune
...
docs/          TECH_STACK.md（技術解説）/ VERIFICATION.md（実機確認手順）
tests/         fixtures生成 / 添付fixtures / bench.py（10万イベント性能計測）
```
