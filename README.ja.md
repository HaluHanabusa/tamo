...
はじめての方は **[15分チュートリアル](docs/TUTORIAL.ja.md)** へ — 実データを触らずに
試せるサンドボックス付きです。
...
詳細・トラブル対処は [browser-extension/README.ja.md](browser-extension/README.ja.md) へ。

## データの場所・保持・削除

- 実体は `~/.tamo/tamo.db`（SQLite 1ファイル）と `~/.tamo/cas/`（添付）。バックアップはフォルダごとコピーでOK
- 既定は**無期限保存**。総サイズが閾値（`[retention] warn_db_mb`、既定2048MB）を超えると
  日次で警告が出ます。`~/.tamo/settings.toml` の `[retention] days = 90` で自動prune
- 手動削除は `tamo prune --days N`（確認プロンプト付き・`--dry-run` で事前確認）、全消去は `tamo purge --yes`
- **保持期間を決める前に [無期限保存のリスクと運用指針](docs/ARCHITECTURE.ja.md) を一読を** —
  平文の全会話を無期限に持つことはセキュリティ/コンプライアンス上のトレードオフです
  （推奨: 業務PCは `days = 90`、`TAMO_HOME` はクラウド同期外、ディスク暗号化前提）
...
| ドキュメント | 内容 |
|---|---|
| [docs/TUTORIAL.ja.md](docs/TUTORIAL.ja.md) | 15分ハンズオン（[English](docs/TUTORIAL.md)） |
| [docs/ARCHITECTURE.ja.md](docs/ARCHITECTURE.ja.md) | 設計原則・データモデル・最適化・還流・保存期間リスク・NFR（[English](docs/ARCHITECTURE.md)） |
| [docs/TECH_STACK.ja.md](docs/TECH_STACK.ja.md) | 技術選定の理由と実装ファイルの対応表（[English](docs/TECH_STACK.md)） |
| [docs/VERIFICATION.ja.md](docs/VERIFICATION.ja.md) | 実機での動作確認手順（[English](docs/VERIFICATION.md)） |
| [browser-extension/README.ja.md](browser-extension/README.ja.md) | ブラウザ拡張の詳細（[English](browser-extension/README.md)） |
