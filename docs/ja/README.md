...
はじめての方は **[15分チュートリアル](TUTORIAL.md)** へ — 実データを触らずに
試せるサンドボックス付きです。
...
詳細・トラブル対処は [ブラウザ拡張のREADME](browser-extension.md) へ。
...
- **保持期間を決める前に [無期限保存のリスクと運用指針](ARCHITECTURE.md) を一読を** —
...
| ドキュメント | 内容 |
|---|---|
| [TUTORIAL.md](TUTORIAL.md) | 15分ハンズオン（[English](../TUTORIAL.md)） |
| [ARCHITECTURE.md](ARCHITECTURE.md) | 設計原則・データモデル・最適化・還流・保存期間リスク・NFR（[English](../ARCHITECTURE.md)） |
| [TECH_STACK.md](TECH_STACK.md) | 技術選定の理由と実装ファイルの対応表（[English](../TECH_STACK.md)） |
| [VERIFICATION.md](VERIFICATION.md) | 実機での動作確認手順（[English](../VERIFICATION.md)） |
| [browser-extension.md](browser-extension.md) | ブラウザ拡張の詳細（[English](../../browser-extension/README.md)） |
